...
def process_ecg():
    try:
        ...
        #     'songs':['https://www.youtube.com/watch?v=DeumyOzKqgI', 'https://www.youtube.com/watch?v=JQbjS0_ZfJ0', 'https://www.youtube.com/watch?v=YykjpeuMNEk']
        # })
        print(results['predictions'])
        songs = []
        for emotion in results['predictions']:
            all_songs = emotion_songs[emotion.lower().strip()]
            all_songs = [song['youtube_link'] for song in all_songs]
            songs.extend(all_songs)
            break
        print(songs)
        return jsonify({
            # 'emotions': results['predictions'],
